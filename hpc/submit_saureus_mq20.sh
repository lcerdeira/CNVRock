...
#
# Run ONLY after the alignment array (job 6376265) has finished, so every
...
#   bash hpc/submit_saureus_mq20.sh

set -euo pipefail
REPO_DIR="$HOME/CNVRock"
BAM_DIR="$REPO_DIR/data/raw/saureus_bam"
LIST="$REPO_DIR/assets/_bams_list_saureus_mq20.txt"

ls "$BAM_DIR"/*.bam > "$LIST"
N=$(wc -l < "$LIST")
echo "S. aureus BAMs to recount at MQ>=20: $N"

sbatch --array=1-"${N}"%50 \
  --export=ALL,\
REFERENCE="$REPO_DIR/assets/saureus_ref/USA300_extended.fasta",\
INTERVALS="$REPO_DIR/assets/saureus_ref/USA300_extended_1kb.interval_list",\
BAM_DIR="$BAM_DIR",\
COUNTS_DIR="$REPO_DIR/data/raw/saureus_readcounts_mq20",\
BAMS_LIST="$LIST",\
MIN_MQ=20 \
